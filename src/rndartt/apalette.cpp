...
constexpr long long kNoMatchDistance = 0x30000;
...
int APalette::FindNearestEntry(unsigned int nColor, int nFirst, int nLast) const {
    ...
    long long nBestDistance = kNoMatchDistance;
    int nBestIndex = 0;
    for (int i = nFirst; i <= nLast; ++i) {
        const unsigned int nEntry = mEntries[i];
        const long long nDeltaRed = nRed - (nEntry & kChannelMask);
        const long long nDeltaGreen = nGreen - ((nEntry >> kGreenShift) & kChannelMask);
        const long long nDeltaBlue = nBlue - ((nEntry >> kBlueShift) & kChannelMask);
        const long long nDistance =
